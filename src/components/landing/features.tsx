 ...
 * Cards share equal heights (h-full on the motion wrapper too), carry a
 * subtle ice-edge top gradient and the engine card shows its timeframes
 * as scannable mono chips.
 */
...
export function Features() {
  ...
  const items = [
    ...
    { icon: BarChart3, title: t.features.engineTitle, desc: t.features.engineDesc, accent: 'text-primary', pills: TIMEFRAMES },
  ...
  return (
    <section id="features" className="relative py-16 sm:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        >
          <span className="inline-flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.25em] text-primary/80 mb-3">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 items-stretch">
          {items.map((item, i) => (
            <motion.div
              ...
              className="h-full"
            >
              <Card className="glass card-interactive card-ice-edge relative overflow-hidden border-border/60 h-full group hover:border-primary/30">
                <CardContent className="p-6 space-y-3">
                  ...
                  >
                    <item.icon className="size-5 icon-bob" strokeWidth={1.75} />
                  </div>
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-0.5">
                      {item.pills.map((tf) => (
                        <span
                          key={tf}
                          dir="ltr"
                          className="rounded-md border border-primary/25 bg-primary/10 px-1.5 py-0.5 font-mono text-[10px] font-bold tracking-wide text-primary/90"
                        >
                          {tf}
                        </span>
                      ))}
