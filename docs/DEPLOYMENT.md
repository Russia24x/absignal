...
| Chart empty / candles `market_data_unavailable` intermittently, track record flips to all-`LOCKED` with 0 wins | GeckoTerminal is rate-limiting the shared Workers egress IP (429 `gt-error-code-429`, sometimes empty 200s) — expected intermittently on the free tier. R38 multi-source chain self-heals this class entirely: candles fall back to **Binance klines** (real OHLCV, keyless — the engine, stats and backtests keep computing through GT outages), the price card falls back to DexScreener → Binance → CoinMarketCap (R35/R38), and both sources persist to the D1 `CandleCache` table (R36, auto-created) so even a total two-venue outage serves stale-flagged data. Durable fixes: a paid GT API key, or set `GECKOTERMINAL_BASE_URL` to a self-hosted proxy |
| `source: "binance"` on the price card / chart badge | Not an error — the R38 fallback chain is doing its job: GeckoTerminal was unavailable and Binance (or another tier) served the snapshot. The badge is honest data provenance |
