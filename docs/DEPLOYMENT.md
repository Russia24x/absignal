...
| Chart empty / candles `market_data_unavailable` intermittently, track record flips to all-`LOCKED` with 0 wins | GeckoTerminal is rate-limiting the shared Workers egress IP (429 `gt-error-code-429`, sometimes empty 200s) — expected intermittently on the free tier. The app self-heals: the overview falls back to DexScreener (R35), and candles persist to the D1 `CandleCache` table (R36, auto-created on first use) so charts, stats, and backtests serve stale-flagged data during outages. First populate needs one successful upstream window — browse the chart once when GT is open. Durable fixes: a paid GT API key, or set `GECKOTERMINAL_BASE_URL` to a self-hosted proxy |
